  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="jumbotron">
		...
		$sql="select name, amount, price from mystore.product where 1";
        echo 'Current Profits: '.$.' <br/> Current Sales: '.$.' <br/><br/>';
		...
		<h4>Employee List</h4>
		<?php
			$sqle="select name,jobtitle, store_id, salary from mystore.salespeople where 1";
			...
			if ($result->num_rows>0)
			{
				...
				while($row = $result->fetch_assoc())
				{
					...
					echo "<td><form action='fire.php' method='POST' ><button name='fire' type='submit' value=$key>Remove Employee</button></form></td>";
					echo "</tr>";
				}
				echo "</table>";
			}
		...
		<form class="form-inline" name="queryInput" action="addnewitem.php" method="POST">
		 
             
        <input type="other" name="itemname" class="form-control" maxlength="25" placeholder="name" required>
			 
            
        <input type="other" name="amount" class="form-control" maxlength="5" placeholder="amount" required>
        ...
        <input type="number" name="energy" class="form-control" maxlength="6" placeholder="energy (calories)" required>
		
	  
        <input type="other" name="fats" class="form-control" maxlength="5" placeholder="fat in grams" required>
        
		
        <input type="other" name="carbs" class="form-control" maxlength="5" placeholder="carbs in grams" required>
		
		
        <input type="other" name="sugars" class="form-control" maxlength="5" placeholder="sugars in grams" required>
		
		
        <input type="other" name="protein" class="form-control" maxlength="5" placeholder="protein in grams" required>
		
		
        <input type="other" name="nutscore" class="form-control" maxlength="3" placeholder="nutrition score" required>
		
		 <input type="other" name="instock" class="form-control" maxlength="5" placeholder="number in stock" required>
		 
		  <input type="other" name="price" class="form-control" maxlength="6" placeholder="price" required>
		...
		<div id="prodsearch">
		 ...
		 <?php 
		  ...
		  $keyword=$_POST["search"];
			...
			$sql="select name,amount,detail,origin,energy,fat,carbohydrates,sugars,proteins,nutritionscore,instocknum,price from mystore.product where 1";
			...
			if ($result->num_rows>0)
			{
				...
				while($row = $result->fetch_assoc())
				{
					...
					echo "<td><form action='removeitem.php' method='POST' ><button name='remove' type='submit' value=$key>Remove from Inventory</button></form></td>";
					echo "</tr>";
				}
				echo "</table>";
			}

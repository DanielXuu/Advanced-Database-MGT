...
<div class="container">
    ...
    <tbody>
		...
		//$itemincrease = $_SESSION['itemincrease'];
		$itemincrease = $_POST['itemupdate'];
		$quant=$_POST["quant"];
		
		
		$sqlstocktest = "Select instocknum from mystore.product where id = $itemincrease";
		$stocktest = $conn->query($sqlstocktest);
		if($stocktest->num_rows> 0){
			while($row = $stocktest->fetch_assoc()){
				if($row['instocknum']-$quant<0){
					echo '<h4><b>Only '.$row['instocknum'].' of item in stock. Please change your order amount.</b></h4>'; 
				}
				else{		
					$sqlincrease="UPDATE mystore.cart SET quantity=$quant WHERE productID = $itemincrease";
					if($conn->query($sqlincrease)===TRUE){
						echo "Item quantity has been increased.";
					}
					else{ 
					echo "Error in quantity increase.";}
				}
			}
		}
		else{

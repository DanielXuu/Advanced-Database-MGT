...
<html lang="en">
  ...
  <body>
   ...
   <div class="container">
      ...
      <div class="jumbotron">
        <h3>Welcome to our online food store!</h3>
		  ...
		  if(isset($_SESSION['username']))
		    echo '<p>You are welcome to browse the online store.</p>';
		  else
   ...
   //$itemtocart = $_SESSION['itemtocart'] ;
   $itemtocart = $_POST['itemadded'];
	...
	echo $itemtocart;
    $sqlinv="select name, instocknum from mystore.product where ID = '$itemtocart'";
	$instock = $conn->query($sqlinv);
	if($instock->num_rows>0){
		
		while($row=$instock->fetch_assoc()){
			
				
	
			if ($row['instocknum']==0){
				echo 'Sorry, item '.$row['name'].' is currently out of stock. <br/> Check back later!';}
   
			else {
				
				$sqlitemamt = "select * from mystore.cart where productid = $itemtocart";
				$itemamt = $conn->query($sqlitemamt);
				if($itemamt->num_rows>0){
					$sqlmore = "update mystore.cart Set quantity = quantity+1 WHERE productID=$itemtocart";
					if($conn->query($sqlmore)===TRUE){
						echo ' Item '.$row['name'].' has been added to your cart! <a href="shoppingcart.php">View Cart.</a>';}
					else{
						echo ' error in adding item';
					}
				}
				else{
					$sqlitemtocart = "insert into mystore.cart values ('$itemtocart','1')";
		
					if($conn->query($sqlitemtocart)===TRUE){
						echo 'Item '.$row['name'].' has been added to your cart! <a href="shoppingcart.php">View Cart.</a>';}
					else{
						echo ' error in adding item';
		}}}

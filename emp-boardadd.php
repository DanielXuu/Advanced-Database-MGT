...
<div class="container">
	...
	$origin=$_POST['origin'];
	$category1=$_POST['cat2'];
	$category2=$_POST['cat1'];
	$energy=$_POST['energy'];
	$fat=$_POST['fats'];
	$carbohydrates=$_POST['carbs'];
	$sugars=$_POST['sugars'];
	$proteins=$_POST['protein'];
	$nutritionscore=$_POST['nutscore'];
	$instocknum=$_POST['instock'];
	$price=$_POST['price'];
	$instocknum=intval($instocknum);


	$sql1="select * from mystore.product where id='$id'";
	$result1 =$conn->query($sql1);
	if ($price<0) {
	echo "<br/>Cannot have a negative Price."; 
	}
	else if($result1->num_rows>0)
	{
        $sqlempremove="DELETE from mystore.product WHERE id='$id'";
        $conn->query($sqlempremove);
        $sql2="insert into mystore.product values ('$id','$name','$amount','$details','$origin','$category1','$category2', '$energy', '$fat', '$carbohydrates', '$sugars', '$proteins', '$instocknum', '$nutritionscore', '$price');";
        $conn->query($sql2);
    ...
    else {echo ' <br/>Item successfully added! ';}
	}
